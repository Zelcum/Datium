...
async function viewSystem(id) {
    ...
    loadingDiv.id = 'viewLoadingScreen';
    loadingDiv.className = 'fixed inset-0 bg-background-light dark:bg-background-dark flex items-center justify-center z-50';
    loadingDiv.innerHTML = `
        <div class="text-center">
            <div class="loading-spinner rounded-full h-12 w-12 mb-4 mx-auto"></div>
            <p class="text-lg font-semibold text-[#111418] dark:text-white fade-in">Cargando sistema...</p>
        </div>
    `;
    ...
    setTimeout(() => {
        const loading = document.getElementById('viewLoadingScreen');
...
async function editSystem(id) {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('systemSecurityMode').value = system.securityMode;
            document.getElementById('formTitle').textContent = 'Editar Sistema';
            handleSecurityModeChange();
            
            const imageUrl = system.imageUrl ? (system.imageUrl.startsWith('http') ? system.imageUrl : `http://localhost:8080${system.imageUrl}`) : null;
            if (imageUrl) {
                document.getElementById('imagePreview').innerHTML = `<img src="${imageUrl}" alt="Preview" class="w-32 h-32 object-cover rounded-xl glass-card p-2">`;
            }
